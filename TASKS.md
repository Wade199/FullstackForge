...
| 21 | og:image custom | 2026-09-23 | Générée via le connecteur Canva (`create-design`) : fond sombre du thème, badge "IW" (dégradé cyan/violet), nom + poste, texte + logo uniquement (pas de photo). 1er essai en carré (1080×1080, le format "Custom size" n'a pas été interprété) — corrigé en régénérant avec le format nommé "YouTube Thumbnail" (1280×720, 16:9). Export impossible via le connecteur (`Not allowed to access design`, 3 tentatives) — téléchargée manuellement par Ibrahima depuis Canva, puis recadrée en local avec Pillow (centre, sans déformation) au format standard **1200×630**. Une seule image pour tout le site (pas une par page). Ajoutée dans `assets/og-image.png` + balises `og:image`/`og:image:width`/`og:image:height`/`twitter:image` sur les 5 pages |
