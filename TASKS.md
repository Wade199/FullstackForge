...
| — | Poste "Développeur Junior" (décision du 2026-09-23) retiré partout, remplacé par "Développeur PHP/Symfony & Flutter" | 2026-09-25 | Ibrahima ne cible plus une alternance et ne veut plus le qualificatif "Junior" — vérifié cohérent avec sa bio GitHub publique (`gh api users/Wade199` → "Développeur PHP/Symfony + Flutter", sans "Junior"). Remplacé : `<title>`, meta description/og/twitter des 5 pages sources (`pages/*.html`), JSON-LD (`jobTitle`), `manifest.json`, `typing` FR/EN/ES dans `i18n/*.json`. Rebuild (`npm run build`) puis hash CSP du JSON-LD recalculé sur le contenu **réellement déployé** (fetché après push, pas le fichier local — même méthode que la tâche #19) et vérifié identique. Même repositionnement appliqué en parallèle sur le CV (Canva) |
