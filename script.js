...
const TRANSLATIONS = {
  ...
  es: {
    ...
    "form-error-email": "❌ Por favor, introduce una dirección de email válida."
  }
};
...
(function initNavbar() {
  ...
  // FIX: utiliser dataset.lang au lieu de lire le textContent (plus robuste avec les emojis)
...
function initReveal() {
  ...
  // Staggered reveal for items — FIX: exclure les .project-card pour éviter double observation
...
function renderProjects(lang) {
  ...
  PROJECTS.forEach((project, index) => {
    ...
    if (imagePath) {
      ...
      img.loading = 'lazy';
      // FIX: cacher l'image ET afficher le placeholder si erreur de chargement
  ...
  grid.querySelectorAll('.reveal-item').forEach((el, i) => {
    // Appliquer un délai staggered sur les project cards aussi
...
function changeLanguage(lang) {
  ...
  // FIX: mettre à jour l'attribut lang du document HTML pour l'accessibilité
  ...
  // FIX: marquer l'option active par data-lang au lieu de lire textContent (plus fiable avec emojis)
...
(function initContactForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    // FIX: validation du format email côté JS
    ...
    // Simuler l'envoi (remplacer par un vrai backend / EmailJS si nécessaire)
    const btn = form.querySelector('.form-submit');
    btn.disabled = true;
    btn.textContent = '⏳ Envoi...';

    // FIX: capturer la langue actuelle pour l'utiliser dans le callback (évite le bug si langue change pendant le timeout)
    const langAtSubmit = currentLang;

    setTimeout(() => {
      const tCallback = TRANSLATIONS[langAtSubmit] || TRANSLATIONS.fr;
      status.textContent = tCallback['form-success'];
      status.className = 'form-status success';
      form.reset();
      btn.disabled = false;
      // FIX: utiliser la langue courante pour le texte du bouton après envoi
      btn.textContent = (TRANSLATIONS[currentLang] || TRANSLATIONS.fr)['form-send'];
      setTimeout(() => {
        status.textContent = '';
...
document.querySelectorAll('a[href^="#"]').forEach(a => {
  a.addEventListener('click', e => {
    ...
    // FIX: vérifier que ce n'est pas juste "#" (lien vide)
    if (href === '#') return;
...
document.addEventListener('DOMContentLoaded', () => {
  // Appliquer la langue sauvegardée (initialise aussi les projects et le typing)
